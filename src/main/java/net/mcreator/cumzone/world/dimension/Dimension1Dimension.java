...
import net.mcreator.cumzone.item.Dimension1Item;
import net.mcreator.cumzone.block.Blockcumzone1Block;
...
public class Dimension1Dimension extends CumzoneModElements.ModElement {
	@ObjectHolder("cumzone:dimension_1_portal")
	public static final CustomPortalBlock portal = null;

	public Dimension1Dimension(CumzoneModElements instance) {
		super(instance, 4);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(Blockcumzone1Block.block);
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("cumzone:biome_3test")).getGenerationSettings().getSurfaceBuilder()
				.get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("cumzone:biome_3test")).getGenerationSettings().getSurfaceBuilder()
				.get().getConfig().getUnder().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("cumzone:biomemod_1")).getGenerationSettings().getSurfaceBuilder()
				.get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("cumzone:biomemod_1")).getGenerationSettings().getSurfaceBuilder()
				.get().getConfig().getUnder().getBlock());
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("cumzone:dimension_1"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("cumzone:dimension_1"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("cumzone:dimension_1")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == Blocks.LAPIS_BLOCK;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? Blocks.LAPIS_BLOCK.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Blocks.LAPIS_BLOCK.getDefaultState(), 3);
					}
				}
			}
